...
def train_model(df_clean):
    ...
    # Use a spinner to show the model is training
    with st.spinner("Training the predictive model... This may take a moment."):
        rf_pipeline.fit(X_train, y_train)
        time.sleep(1)  # Simulate a bit more work
...
with st.container():
    ...
    st.markdown(
        f"<div class='verdana-text'>The **RMSE** of **${rmse:,.2f}** means that, on average, the model's prediction for a premium is off by about this amount.</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"<div class='verdana-text'>The **R²** of **{r2:.3f}** indicates that the factors used in the model can explain **{r2:.1%}** of the variation in insurance premiums.</div>",
        unsafe_allow_html=True,
    )
    
# Container for EDA plots, with expanders for better organization
with st.container():
    st.subheader("Data Insights")
    
    with st.expander("Correlation Heatmap & Distribution of Charges"):
    ...
    with st.expander("Charges by Smoker Status & Region"):
    ...
    with st.expander("Interactive BMI vs Charges Plot"):
...
# Container for model diagnostics plots
with st.container():
    st.subheader("Model Diagnostics")

    with st.expander("Actual vs Predicted Charges & Feature Importances"):
